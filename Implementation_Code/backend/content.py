...
def handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET' and '/explain/' in path:
            ...
            # Get topic context by searching StudyPlans to extract exact titles
            topic = None
            try:
            ...
            if not topic:
                topic = {'title': topic_id, 'description': ''}
...
3.explaination(like relating some real life situation or senario to the mechanisim or principles like give imagine you are someting ... if possible to this topic)
...
2. Use bullet lists with "-" only.
Example:
- First point
- Second point

Never use the bullet symbol "•".

3. Use **bold text** for important terms.

4. Mathematical equations must use LaTeX with:
Inline math:
\\( equation \\)
...
6. Do NOT include code blocks like ```markdown```.

7. Avoid unusual characters.


{hinglish_prompt if language == 'hinglish' else 'Use clear English.'}"""

            content = invoke_model(prompt, {'maxTokens': 10000, 'temperature': 0.7})
...
Keep the answer concise (under 200 words)."""
        ...
        if http_method == 'GET' and '/references/' in path:
            ...
            prompt = f"""Generate 3-5 high-quality external learning resources which are availabe now for the topic: {topic_title}

For each resource, provide:
1. Title
2. Type (article/video/documentation)
3. URL (use well-known educational platforms like  W3Schools, popular youtube videos)
